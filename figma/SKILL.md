...
# 1. Сначала получи nodeId нужного элемента
curl -s -X POST "http://localhost:4000/figma/command" \
  -H "Content-Type: application/json" \
  -d '{"user_id":"USER_ID","command":{"type":"set-effects","nodeId":"NODE_ID","action":"get"}}'

# 2. Через 2-3 сек забери ответ
curl -s "http://localhost:4000/figma/response/USER_ID?wait=1"
...
## 16. Клонировать элемент (копировать)

Создаёт точную копию элемента со всеми свойствами (размер, цвет, эффекты, дети).
Возвращает `cloneId` — id нового элемента, с которым можно сразу работать дальше.

```bash
curl -s -X POST "http://localhost:4000/figma/command" \
  -H "Content-Type: application/json" \
  -d '{"user_id":"USER_ID","command":{"type":"clone","nodeId":"NODE_ID","offsetX":20,"offsetY":0,"name":"Logo copy"}}'
```

```bash
# Забрать cloneId (нужен для последующих команд move, resize и т.д.)
curl -s "http://localhost:4000/figma/response/USER_ID?wait=1"
```

Параметры:
- `nodeId` — что клонировать
- `offsetX`, `offsetY` — смещение клона относительно оригинала (по умолч. 20/20)
- `name` — имя клона (по умолч. "имя оригинала (copy)")
- `parentId` — куда поместить клон (по умолч. тот же контейнер что у оригинала)

Ответ содержит `cloneId`, `cloneName`, `x`, `y` — используй `cloneId` для move/resize/etc.

### Типичный сценарий: скопировать и переместить

1. `get-page-nodes` с nameFilter → найти nodeId лого
2. `clone` с нужным offsetX/offsetY → получить cloneId из response
3. `move` с cloneId → переместить в нужную позицию

---

## 17. Переименовать элемент
...
## 18. Переместить элемент внутрь другого
